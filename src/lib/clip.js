...
export class Clip {
  ...
  whenReady() {
    if (this.el.readyState >= 2 || this.ready) return Promise.resolve(this);
    return new Promise((res) => {
      const done = () => { cleanup(); this.ready = true; res(this); };
      const cleanup = () => {
      ...
      this.el.addEventListener('loadeddata', done, { once: true });
      ...
      // Safety timeout: never hang more than 1.5s
