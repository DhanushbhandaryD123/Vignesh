...
export class Finale {
  constructor(back, glB, front, glF) {
    ...
    this.texF = { grain: texture(glF, { wrap: 'repeat' }) };
    this.par = { x: 0, y: 0 };
    this.manBox = [540, 105, 530, 836];
  }

  async load() {
    let plate;
    try {
      plate = await loadImage('public/fin/Footer image.png');
    } catch {
      plate = await loadImage('public/fin/plate.jpg');
    }
    const grain = await loadImage('public/tex/grain.png');
