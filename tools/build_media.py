...
CLIPS = [
    ("hero", "v2.mp4", "v2", 720, 1280, 0, 192),
    # ("v1", "v1.mp4", "v1", 720, 1280, 0, 240),
...
def build_grunge():
    ...
    """
    typo_path = os.path.join(SRC, "ref_typo.png")
    if not os.path.exists(typo_path):
        typo_path = os.path.join(SRC, "ref_typo.jpg")
    if not os.path.exists(typo_path):
        return
    img = cv2.imread(typo_path)
    if img is None:
        return
