...
def frame_alpha(bgr, cfg):
    ...
    # Clean cyclorama floor reflections around dark shoes
    if cfg.get("clean_floor", False):
        floor_y = int(h * (1.0 - cfg.get("floor_guard", 0.12)))
        floor_refl = (np.arange(h)[:, None] > floor_y) & (L > 120)
        a[floor_refl] = 0.0

    # Preserve face and teeth solid
    if cfg.get("preserve_face", False):
        head_box = np.zeros((h, w), dtype=bool)
        head_box[int(h * 0.06):int(h * 0.36), int(w * 0.35):int(w * 0.65)] = True
        face_zone = ((dC > 8) | (L < 80)) & head_box
        face_closed = cv2.morphologyEx(face_zone.astype(np.uint8), cv2.MORPH_CLOSE, np.ones((25, 25), np.uint8))
        face_filled = fill_holes(face_closed, 10000).astype(bool) & head_box
        a[head_box & face_filled] = np.maximum(a[head_box & face_filled], 1.0)
...
PRESETS = {
    ...
    # dark suit on bright cyclorama (Vignesh)
    "v2": dict(ring=14, ring_min_L=150, w_lum=1.0, w_chroma=1.6, lo=12, hi=44,
               bg_thr=12, fg_thr=46, edge_barrier=0, seed_edges="ltrb",
               band=20, seal=9, hole_min=0, gf_radius=4, gf_eps=1e-4, choke=0.06,
               out_lo=0.15, out_hi=0.96, despill=1.0, unmul_floor=0.22,
               est_win=17, ema=0.55, floor_guard=0.12,
               clean_floor=True, preserve_face=True),
